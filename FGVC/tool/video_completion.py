...
def video_completion(args):
    ...
    for i in range(nFrame):
        ...
        imageio.mimwrite(os.path.join(args.outroot, 'frame_comp_' + 'final', 'final.mp4'), video_comp_, fps=25, quality=8, macro_block_size=1)
...
def video_completion_seamless(args):
    ...
    for i in range(nFrame):
        ...
        imageio.mimwrite(os.path.join(args.outroot, 'frame_seamless_comp_' + 'final', 'final.mp4'), video_comp_, fps=args.outputFps, quality=8, macro_block_size=1)
...
def initargs(path,path_mask,outroot,outputFps=25):
    args=argparse.Namespace(H_scale=2, Nonlocal=False, W_scale=2, alpha=0.1, alternate_corr=False, consistencyThres=inf, deepfill_model='../weight/imagenet_deepfill.pth', edge_completion_model='../weight/edge_completion.pth', edge_guide=False, mixed_precision=False, mode='object_removal', model='../weight/raft-things.pth', outputFps=outputFps, outroot=outroot, path=path, path_mask=path_mask, seamless=False, small=False)
if __name__ == '__main__':
    ...
    parser.add_argument('--outputFps', default=25, type=float)
